...
export const GalleryController = ({ buttonClass, buttonStyle, item }) => {
  ...
  const images = item?.url?.split(",").filter(Boolean) || [];
  const gridCols = images.length >= 3 ? 3 : images.length;
  ...
  // ✅ always render something, even if it's empty
  if (!item?.url) {
    return <div />; // or loading spinner, empty state, whatever you want
  }
  ...
  return (
    ...
    >
      <div
        className={`grid grid-cols-${gridCols} gap-2 rounded-lg mx-auto`}
        style={{ width: gridCols === 1 ? "100%" : "100%" }}
      >
      ...
      {currentIndex !== null && (
        <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50">
          <button
            onClick={closeViewer}
            tabIndex={1}
            className="absolute top-4 right-4 text-white text-2xl p-5"
          >
            ✖
          </button>
          <button
            onClick={showPrev}
            tabIndex={2}
            className="absolute left-4 text-white text-4xl p-5"
          >
            ⟨
          </button>

          {images[currentIndex].endsWith(".webm") || images[currentIndex].endsWith(".mp4") ? (
            <video
          ...
          <button
            onClick={showNext}
            tabIndex={3}
            className="absolute right-4 text-white text-4xl p-5"
          >
            ⟩
          </button>
        </div>
...
export const VimeoController = ({ item }) => {
  ...
  // Get container width and calculate height (16:9)
  useEffect(() => {
  ...
  return (
    <div ref={containerRef} className="flex flex-col justify-center items-center h-auto w-full">
      {error ? (
        <p className="text-red-500">{error}</p>
      ) : vimeoEmbedUrl ? (
        <>
...
export const MobileAppController = ({ item }) => {
  ...
  return (
    <div className="bg-white border rounded-xl shadow-lg p-6 mx-auto w-full max-w-2xl flex flex-row items-center justify-center gap-4">
      {apkList?.selectedImage?.group === "group1" ? (
        <>
          {apkList?.appstore_link && (
            <a href={apkList?.appstore_link} target="_blank" rel="noopener noreferrer">
              <img
                src="/images/bio/app-store-dark-button.svg"
                alt="App Store"
          ...
          {apkList?.playstore_link && (
            <a href={apkList?.playstore_link} target="_blank" rel="noopener noreferrer">
              <img
                src="/images/bio/play-store-dark-button.svg"
                alt="App Store"
                className="w-48 h-auto cursor-pointer"
              />
            </a>
        ...
        <>
          {apkList?.appstore_link && (
            <a href={apkList?.appstore_link} target="_blank" rel="noopener noreferrer">
              <img
                src="/images/bio/app-store-light-button.svg"
                alt="App Store"
                className="w-48 h-auto cursor-pointer"
              />
            </a>
          ...
          {apkList?.playstore_link && (
            <a href={apkList?.playstore_link} target="_blank" rel="noopener noreferrer">
              <img
                src="/images/bio/play-store-light-button.svg"
                alt="App Store"
                className="w-48 h-auto cursor-pointer"
              />
            </a>
