...
const ThirdPartyControllerShowAsButton = ({
  ...
  useEffect(() => {
    if (containerRef.current) {
      ...
      // or: containerRef.current.getBoundingClientRect().width
    }
  }, []);

  const getVideoSrc = () => {
    return item?.app_name === "youtube" ? ConvertYouTubeURL(item?.url) : item?.url;
  };

  const iframeHeight = divWidth ? (divWidth * 9) / 16 : 0;
