  ...
  return async (dispatch) => {
    try {
      ...
      if (apiCall?.status === 200 || apiCall?.status === 304) {
        ...
        // if (!apiCall?.data?.data?.social_login) {
  ...
  return async (dispatch) => {
    try {
      ...
      if (apiCall?.status === 200 || apiCall?.status === 304) {
        ...
        dispatch(getUser());
        dispatch(getBio(apiCall?.data?.data?.username));
...
export const addReportUser = (data) => {
  return async (dispatch) => {
    try {
      dispatch({
        type: LOADER,
      ...
      if (apiCall?.data?.code === 201 || apiCall?.data?.status === 304) {
        dispatch({
          type: LOADER,
          payload: false,
        });
...
export const deleteWidget = (id, userName) => {
  return async (dispatch) => {
    try {
      dispatch({
        type: LOADER,
      ...
      if (apiCall?.data) {
        ...
        ToastNotification.success("Widget delete successfully!");
        dispatch(getBio(userName));
      }
    } catch (error) {
      dispatch({
...
export const createWidget = (data, userName) => {
  return async (dispatch) => {
    try {
      dispatch({
        type: LOADER,
